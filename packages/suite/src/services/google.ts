...
class Client {
    ...
    static setClientIdForFlow(flow: 'offline' | 'online') {
        if (flow === 'offline') {
            ...
            if (isDesktop()) {
                console.log('using desktop id');
                // Client.clientId = METADATA.GOOGLE_CLIENT_ID_DESKTOP; // unsupported_response_type
                Client.clientId = METADATA.GOOGLE_CLIENT_ID_WEB; // #hash..
            } else {
                console.log('using web id');
    ...
    static async init(token = '') {
        console.log('init google token', token);
        ...
        if (token) {
            if (Client.flow === 'offline') {
                console.log('init setCredentials isAuthServerAvailable', Client.token);
                Client.oauth2Client.setCredentials({
                    refresh_token: Client.token,
                });
            } else {
                console.log('init setCredentials', Client.token);
     ...
     */
    static revoke() {
        console.log('Client.revoke', Client.token);
        if (!Client.token) return;
        const promise = Client.call(`https://oauth2.googleapis.com/revoke?token=${Client.token}`, {
            method: 'POST',
        });
